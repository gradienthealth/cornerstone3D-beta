...
function parseImageId(imageId: string): CornerstoneImageUrl {
  // build a zip url by parsing out the url scheme, dicom file and frame index from the imageId
  ...
  const dicomFileIndex = url.lastIndexOf('/');
  const dicomFile = url.substring(dicomFileIndex + 1);
  url = url.substring(0, dicomFileIndex);
